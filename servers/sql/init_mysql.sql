...
CREATE TABLE USERS (
  ...
  FOREIGN KEY manager_user_id(`manager`) REFERENCES USERS(`id`) ON UPDATE CASCADE ON DELETE CASCADE,
  `op_v_user`   BOOLEAN NOT NULL DEFAULT FALSE,
  `op_v_quest`  BOOLEAN NOT NULL DEFAULT ,
  `op_c_match`  BOOLEAN NOT NULL DEFAULT FALSE
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE QUESTIONS (
  ...
  FOREIGN KEY owner_id(`owner`) REFERENCES USERS(`id`) ON UPDATE CASCADE ON DELETE CASCADE,
  `verified` BOOLEAN NOT NULL DEFAULT FALSE
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE MATCH_USERS (
  ...
  FOREIGN KEY muser_id(`userid`) REFERENCES USERS(`id`) ON UPDATE CASCADE ON DELETE CASCADE
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
